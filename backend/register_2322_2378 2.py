...
def register_2322_2378():
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        # Category Logic
        cat_id = 2 # Default? Or Living?
        
        cat_str = str(category_name).lower() if category_name else ""
        m_lower = str(model_name).lower()
        
        # Priority mapping
        if 'food' in cat_str or '식품' in cat_str or '짜장' in m_lower or '김치' in m_lower or '소스' in m_lower:
            cat_id = 5 # Food
        elif 'mobile' in cat_str or '충전기' in m_lower or 'charger' in m_lower or 'powerbank' in m_lower or '셀카' in m_lower:
            cat_id = 2 # Mobile
        elif 'travel' in cat_str:
            # Check for toothbrush sterilizer
            if '칫솔' in m_lower or 'sterilizer' in m_lower:
                cat_id = 10 # Living
            else:
                cat_id = 10 # Default Travel to Living/Other
        elif 'electronics' in cat_str:
            cat_id = 12
