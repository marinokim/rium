...
def register_2146_2286():
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        # Category Logic
        cat_id = 12 # Default to Electronics (likely Shinil)
        ...
        if 'coffee' in cat_str:
            cat_id = 5 # Food
        elif 'mobile' in cat_str or '이어폰' in m_lower or 'earphone' in m_lower:
            cat_id = 2 # Mobile (Tech)
        elif 'beauty' in cat_str or '화장품' in m_lower or 'cosmetic' in m_lower:
            cat_id = 3
        
        # Electronics vs Kitchen
        if '후라이팬' in m_lower or '그릴' in m_lower or 'grill' in m_lower or '두유제조기' in m_lower or 'kitchen' in cat_str:
            cat_id = 11 # Kitchen
        elif 'electronics' in cat_str or '신일' in str(brand):
            # Shinil can be Electronics or Living or Kitchen
            # If not kitchen, stick to 12? Or 10?
            # User previously had Living. 
            # But let's use 12 since we have it now and items are "Electronics".
            # Unless it's a heater/fan - usually Living (10).
            if '히터' in m_lower or '난로' in m_lower or '선풍기' in m_lower or '가습기' in m_lower or 'humidifier' in m_lower:
                cat_id = 10 # Living (Home Appliances often classed as Living)
            else:
                cat_id = 12 # Electronics generic
