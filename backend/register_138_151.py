...
def register_138_151():
    ...
    # Process rows 138 to 151
    # We iterate 138 to 151 inclusive
    min_r = 138
    max_r = 151
    print(f"Processing rows {min_r} to {max_r}...")
    
    row_idx = min_r - 1 # Just for counter logic if needed, openpyxl uses 1-based
    updated_count = 0
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        # Default to Gift Set (8) if unknown, or try to logic it?
        # User hasn't specified category, so let's default to Gift Set (8) 
        # BUT if category_name is present, maybe we should use it?
        # For this specific task (Gogayeon/Plbirak), they look like food/gift sets.
        # I will start with 8 (Gift Set) to be consistent with previous batch unless user specified.
        # Actually, let's look at the previous batch logic, it forced cat_id=8.
        # I'll stick to cat_id=8 for safety unless category_name suggests otherwise strongly.
        
        cat_id = 8
        if '건강' in str(model_name): cat_id = 8 # Assumption
        
        # Just use 8 for now as "General/Unsorted" or "Gift Set"
        ...
        try:
            if target_id:
                ...
                print(f"Row {row_idx}: Updated Product {target_id} ({model_name})")
            else:
                # Insert
                con.run(
