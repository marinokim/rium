...
def register_2493_2553():
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        # Verification check
        # If weird outlier appears, we can add logic, but inspected rows are all audio.
        # Just in case:
        if 'food' in str(category_name).lower():
            cat_id = 5
        elif 'beauty' in str(category_name).lower():
            cat_id = 3
