...
def register_1758_1798():
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        # Priority mapping
        if 'coffee' in cat_str or '커피' in str(model_name) or '라바짜' in str(model_name) or '네스프레소' in str(model_name) or '돌체구스토' in str(model_name):
            cat_id = 5 # Food
        elif '이어폰' in str(model_name) or 'earphone' in str(model_name) or '카메라' in str(model_name) or 'camera' in str(model_name) or '모비프렌' in str(model_name) or '톤웨이블' in str(model_name) or 'MobiFren' in str(model_name):
            cat_id = 2 # Mobile (Tech)
        elif '발냄새' in str(model_name) or 'coolfoot' in str(model_name) or '쿨풋' in str(model_name) or '화장품' in str(model_name) or 'beauty' in cat_str:
            cat_id = 3 # Beauty
