...
def register_2426_2461():
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        # Category Logic
        cat_id = 2 # Default to Mobile
        
        # Check explicit brand
        if 'pantech' in str(brand).lower() or '팬택' in str(brand):
            cat_id = 2
        elif 'moodon' in str(brand).lower() or 'tumbler' in str(model_name).lower() or '텀블러' in str(model_name).lower():
            cat_id = 10 # Living
        elif 'inkel' in str(brand).lower():
            # Inkel speaker -> Mobile (2)
             cat_id = 2
        
        cat_str = str(category_name).lower() if category_name else ""
        if 'living' in cat_str:
            cat_id = 10
