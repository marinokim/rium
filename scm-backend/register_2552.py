...
def register_2552():
    ...
    # Lookup Category 'Audio'
    print("Looking up category 'Audio'...")
    cat_rows = con.run("SELECT id FROM categories WHERE name = 'Audio' OR slug = 'audio'")
    if cat_rows:
        cat_id = cat_rows[0][0]
        print(f"Found Category ID for Audio: {cat_id}")
    else:
        print("Category 'Audio' not found. It might need to be created manually or mapped.")
        # Try finding 'Digital' or similar? 
        # But for now let's assume it exists or fail.
        # Actually, let's list to see if we missed it.
        all_cats = con.run("SELECT id, name FROM categories")
        print(f"Available Categories: {all_cats}")
        return
