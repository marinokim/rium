...
def register_1136_1154():
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        # Category Logic
        cat_id = 8 # Default to Gift Set
        
        if category_name:
            cat_str = str(category_name).lower()
            if 'kitchen' in cat_str or 'living' in cat_str:
                cat_id = 10 # Living (contains Kitchen)
            elif 'gift' in cat_str or '선물' in cat_str:
                cat_id = 8
            else:
                # Based on user pattern, prefer Living/Kitchen if ambiguous or default?
                # For now default to 10 if it looks like general goods, assuming this batch is uniform.
                # But safer to stick to 10 only if matched.
                if 'kitchen' in cat_str:
                    cat_id = 10
