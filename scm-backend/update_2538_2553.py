...
def sanitize(val, allow_html=False):
    ...
    if not allow_html:
        # If it's an img tag, extract src
        if '<img' in s:
...
def update_2538_2553():
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        # Allow HTML for DetailURL & try to extract src if it's just an img tag, 
        # BUT if it's a full HTML block, keep it. 
        # The user request specifically mentioned "Detail Image" issue.
        # The inspection showed simple URLs or simple img tags.
        # IF it's a direct URL string (http...), we use it.
        # IF it's an <img> tag, we extract src if we want just the URL, OR we keep the tag if we want HTML.
        # Usually `DetailURL` in the DB is just the URL string if it's a single image, OR a comma-separated list, OR HTML.
        # Previously we allowed HTML `sanitize(..., allow_html=True)`.
        # However, for the specific user request about "Detail Image showing up", 
        # if the column is used as an `src` in frontend, it needs to be a URL. 
        # If the column is rendered as HTML, it needs to be HTML.
        # Looking at previous logs, `DetailURL` is often treated as a list of images or a URL.
        # But wait, looking at the inspection: `DetailURL: https://kdsound...`
        # This looks like a direct URL.
        # Let's try to extract a clean URL if possible, or keep the value if it's HTML.
        # Actually, for GURU item (Q10), the sample showed a direct URL.
        # For TEAC, it showed `<img ... src="...">`.
        # I will use a robust strategy: If it looks like a URL, use it. If it's an img tag, extract src.
        # This matches the `extract_src` logic which is robust.
        # BUT `detail_url` column in DB might expect HTML? 
        # In `register_1081_1111.py` I used `allow_html=True`.
        # Here I will try to extract the clean URL for consistency if it's just a single image wrapping.
        # If the user wants the image to "show", likely the frontend expects a URL or the HTML is broken.
        # Since the Excel now has valid absolute URLs, let's prioritize extracting the valid URL `http...` from whatever mess is there.
        
        raw_detail = get_val(['DetailURL', '상세페이지URL'])
        detail_url = extract_src(raw_detail) # This will get the URL from img tag or the string itself
        if not detail_url:
             detail_url = sanitize(raw_detail, allow_html=True)

        # Image URL with <img> parsing
        raw_image_url = get_val(['ImageURL', '이미지URL'])
        image_url = truncate(extract_src(raw_image_url))
        if not image_url:
            image_url = truncate(sanitize(raw_image_url))
                ...
                con.run(
                    """UPDATE products 
                       SET model_name = :m_name,
                           ...
                           detail_url = :detail, -- Force update detail_url
