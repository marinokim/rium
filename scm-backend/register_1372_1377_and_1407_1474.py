    ...
    for min_r, max_r in ranges:
        ...
        for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
            ...
            if category_name:
                ...
                # Special cases based on Brand/Name if category is vague
                if '해피콜' in str(model_name) or '쿡셀' in str(model_name) or '테팔' in str(model_name):
                    cat_id = 11 # Kitchen force
        ...
        print(f"Range {min_r}-{max_r} done. Processed {updated_count} items.")

    print(f"All ranges done.")
    con.close()
