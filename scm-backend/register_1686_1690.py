...
def register_1686_1690():
    ...
    # Lookup Category 'Living'
    print("Looking up category 'Living'...")
    cat_rows = con.run("SELECT id FROM categories WHERE name = 'Living' OR slug = 'living'")
    if cat_rows:
        cat_id = cat_rows[0][0]
        print(f"Found Category ID for Living: {cat_id}")
    else:
        print("Category 'Living' not found. Using default ID 10 (Living) as fallback assumption or erroring.")
        # Let's try to list categories if not found to be safe, but mostly it should be there.
