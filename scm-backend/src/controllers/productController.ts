...
export const getProducts = async (req: Request, res: Response) => {
    try {
        try {
            const { isNew, limit, page, includeUnavailable, categoryId, search } = req.query;
            ...
            // Filter by Search (Name or ModelNo)
            if (search) {
                where.OR = [
                    { name: { contains: String(search) } },
                    { modelNo: { contains: String(search) } },
                    { brand: { contains: String(search) } } // Optional: also search brand
                ];
            }
