...
def register_2287_2321():
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        # Category Logic
        cat_id = 10 # Default to Living
        
        cat_str = str(category_name).lower() if category_name else ""
        m_lower = str(model_name).lower()
        
        if 'electronics' in cat_str:
            cat_id = 12
        elif 'living' in cat_str:
            cat_id = 10
        
        # Override based on name/brand
        if '코맥스' in str(brand) or 'komax' in str(brand).lower() or '고무장갑' in m_lower or '수세미' in m_lower or '지퍼백' in m_lower:
             # Komax items (gloves, etc) are often kitchen
             cat_id = 11 # Kitchen
        elif '차량용' in m_lower and '청소' not in m_lower and '코팅' not in m_lower:
             # Car accessories often Mobile or Living..
             # But here 'Auto Guardian' is coating agent -> Living is fine (default).
             pass
        elif '마사지' in m_lower:
             # Massage gun -> Electronics (12) or Beauty (3).
             # If it was labeled Electronics, it stayed 12. If Living, it supports 10.
             # Let's prefer Electronics if it's a 'Gun' (machine).
             cat_id = 12
