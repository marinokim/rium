...
def register_2084_2145():
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        if 'coffee' in cat_str:
            cat_id = 5 # Food
        elif 'mobile' in cat_str or '보조배터리' in str(model_name) or 'power bank' in str(model_name).lower() or '이어폰' in str(model_name) or 'earphone' in str(model_name):
