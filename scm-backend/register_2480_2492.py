...
def register_2480_2492():
    ...
    for row in sheet.iter_rows(min_row=min_r, max_row=max_r, values_only=True):
        ...
        b_lower = str(brand).lower()
        
        # Geneva HighEnd Audio
        if 'geneva' in b_lower or 'audio' in cat_str:
            cat_id = 1 # Audio
        # HIVE Speaker
        elif 'hive' in b_lower or 'speaker' in m_lower or '스피커' in m_lower:
            cat_id = 2 # Mobile (or Audio 1? Let's use 2 for mini portable items, 1 for premium)
        # Shinil Toothbrush
        elif '칫솔' in m_lower or 'toothbrush' in m_lower:
            cat_id = 10 # Living (Healthcare/Hygiene)
